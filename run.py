...
from exp.exp_long_term_forecasting_rptf import Exp_Long_Term_Forecast_RPTF
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='RPTF')
    ...
    parser.add_argument('--model', type=str, required=True, default='RPTF',
                        help='model name, options: [RPTF_Llama, RPTF_Gpt2, RPTF_Opt1b, RPTF]')
    ...
    # RPTF model parameters (Section 3)
    parser.add_argument('--pooling_kernels', type=int, nargs='+', default=None,
                        help='multi-resolution pooling kernels, e.g., 1 4 8 24 168')
    parser.add_argument('--period_list', type=int, nargs='+', default=None,
                        help='detected period list for periodic encoding, e.g., 24 168')
    parser.add_argument('--max_offset_d', type=int, default=96,
                        help='maximum offset D for relative temporal encoding')
    parser.add_argument('--basis_dim', type=int, default=64,
                        help='basis dimension for periodic encoding')
    parser.add_argument('--lambda_scale', type=float, default=0.5,
                        help='loss weight lambda for scale-specific vs aggregated loss (Equation 17)')
    parser.add_argument('--eta_entropy', type=float, default=0.01,
                        help='entropy regularization coefficient (Equation 18)')
    ...
    if args.task_name == 'long_term_forecast':
        if args.model == 'RPTF':
            Exp = Exp_Long_Term_Forecast_RPTF
        else:
