...
class Exp_Long_Term_Forecast_RPTF(Exp_Basic):
    """
    RPTF: Relative-Periodic Temporal Fusion
    Long-term forecasting experiment with multi-scale temporal modeling.

    Training pipeline implements the complete objective from Equations 16-19:
        L_total = λ·Σ_s L_s + (1-λ)·L_agg - η·Σ_s w_s·log(w_s)

    Key differences from standard long-term forecasting:
    1. Uses forward_with_loss() instead of standard forward() during training
    2. Logs per-scale losses and dynamic weights for monitoring
    3. Scale-specific and aggregated predictions are jointly optimized
    """

    def __init__(self, args):
        # Detect periods and build multi-resolution schedule before init
        if not hasattr(args, 'pooling_kernels') or args.pooling_kernels is None:
            from models.RPTF import build_multi_resolution_schedule
            args.pooling_kernels = build_multi_resolution_schedule(
                dataset_name=args.data,
                seq_len=args.seq_len,
                dom_period=None
            )
        if not hasattr(args, 'period_list') or args.period_list is None:
            # Default periods per dataset
            if args.data in ['ETTh1', 'ETTh2', 'ECL', 'electricity']:
                args.period_list = [24]
            elif args.data in ['ETTm1', 'ETTm2']:
                args.period_list = [96]
            elif args.data in ['weather', 'Weather']:
                args.period_list = [144]
            elif args.data in ['Solar', 'solar', 'Solar-Energy']:
                args.period_list = [144]
            elif args.data in ['traffic', 'Traffic']:
                args.period_list = [24]
            else:
                args.period_list = [24]

        super(Exp_Long_Term_Forecast_RPTF, self).__init__(args)

    def _build_model(self):
        """Build the RPTF forecasting model with multi-resolution capabilities."""
        from models import RPTF
        model = RPTF.Model(self.args)
    ...
    def _select_optimizer(self):
        ...
        model_optim = optim.AdamW([{'params': p_list}], lr=self.args.learning_rate,
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                # Use forward_with_loss during validation
                if hasattr(self.model, 'module'):
                    result = self.model.module.forward_with_loss(
                        batch_x, batch_x_mark, None, batch_y_mark, target=batch_y.to(self.device)
                    )
                else:
                    result = self.model.forward_with_loss(
                        batch_x, batch_x_mark, None, batch_y_mark, target=batch_y.to(self.device)
                    )
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # Use forward_with_loss for multi-scale training
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if hasattr(self.model, 'module'):
                            total_loss, outputs, scale_losses, weights = self.model.module.forward_with_loss(
                                batch_x, batch_x_mark, None, batch_y_mark, target=batch_y
                            )
                        else:
                            total_loss, outputs, scale_losses, weights = self.model.forward_with_loss(
                                batch_x, batch_x_mark, None, batch_y_mark, target=batch_y
                            )
                else:
                    if hasattr(self.model, 'module'):
                ...
                if (i + 1) % 100 == 0:
                    if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                        print("\titers: {0}, epoch: {1} | total_loss: {2:.7f}".format(
                            i + 1, epoch + 1, total_loss.item()))
                        # Print scale-specific losses
                        for k, v in scale_losses.items():
                            print("\t\t{}: {:.7f}".format(k, v.item()))
                        print("\t\tweights: {}".format(
                            [f"{w:.4f}" for w in weights.detach().cpu().numpy()]))
                        speed = (time.time() - time_now) / iter_count
    ...
    def test(self, setting, test=0):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                for j in range(inference_steps):
                    ...
                    if self.args.use_amp:
                        with torch.cuda.amp.autocast():
                            if hasattr(self.model, 'module'):
                                outputs = self.model.module(batch_x, batch_x_mark, None, batch_y_mark)
                            else:
        ...
        f = open("result_long_term_forecast_rptf.txt", 'a')
        f.write(setting + "  \n")
